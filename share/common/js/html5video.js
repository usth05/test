		...
		event: function() {
			...
			mui_video.addEventListener("ended", function() {
				if(_this.options.iospay && mui.os.ios) {
					...
					document.getElementById("myBg").style.cssText = "position: absolute;left: 0;top: 0;width: 100%;height: 4rem;z-index: 7;display: block;"
			...
			var isfull = function(isstat) {
				if(fullScreenFlag) {
					...
					if(mui.os.ios) {
						//IOS
						_this.getDom.style.cssText = "background-color: black;overflow: hidden;width: 100%;height: 4rem;position: relative;";
						if(mui.os.plus) {
			...
			mui_full_back.addEventListener("tap", function() {
				...
				if(mui.os.ios) {
					//IOS
					_this.getDom.style.cssText = "background-color: black;overflow: hidden;width: 100%;height: 4rem;position: relative;";
					document.getElementById("myBack").style.cssText = "color: white;position: absolute;left: 0.15rem;top: 0.35rem;z-index: 999;font-size: 0.6rem;display: block;"
					if(mui.os.plus) {
						plus.navigator.setFullscreen(false);
						plus.screen.lockOrientation('portrait'); //锁死屏幕方向为竖屏
					}
